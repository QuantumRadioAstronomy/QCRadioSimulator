...
class Simulation:
    ...
    def make_qpie_img(self,img, ns):
        ...
        samples = np.random.choice(np.arange(N2), ns, p=c)
        for s in samples:
            img_sampled[s] += 1
    ...
    def make_obs(self, nsource, nshots,showplot = False):
        ...
        if showplot:
            ...
            plt.savefig("example_noise{0:.0f}.pdf".format(self.noise_level*100))
            plt.show()
...
def eff_vs_noise(noise_levels, nshots, encoding = 'QPIE'):

    eff_cc = []
    eff_qc = {}
    for ns in nshots: eff_qc[ns] = []

    for ni in noise_levels:
        d_cc, d_qc = run_many_simulations(100,nshots = nshots, noise = ni, encoding = encoding, showplot = False)

        d = np.array(d_cc)
        eff_cc.append(len(d[d < 1.5])*1./len(d))
        for ns in nshots:
            d = np.array(d_qc[ns])
            eff_qc[ns].append(len(d[d < 1.5])*1./len(d))

    f, ax = plt.subplots()
    plt.plot(noise_levels,eff_cc,c='lightskyblue',marker='o',label ="Original")
    qc_color = {10:'red', 100:'orchid', 1000:'navy', 10000:'dodgerblue'}
    ...
    for ns in nshots[::-1]:
        plt.plot(noise_levels,eff_qc[ns],c=qc_color[ns], **qc_style[ns], label ="{1} Reco. w/ {0} shots".format(ns, encoding))
    plt.xlabel("Injected Gaussian noise [% of maximum pixel value]")
    plt.ylabel("Source ID Efficiency")
    ax.set_ylim(0,1.2)
    plt.legend(frameon=False)
    plt.savefig("eff_vs_noise.pdf")
    plt.show()

def eff_vs_imsize(imsize, noise = 0.1, encoding = 'qpie'):
    ...
    for ims in imsize:
        nsource = int(ims*ims*4/(32*32))+1
        d_cc, ds_qc = run_many_simulations(100, nsource = nsource,
                                                nshots = [int(ims*ims/10), int(ims*ims/2), ims, ims*ims],
        ...
        d = np.array(ds_qc[ims*ims])
        eff_qc.append(len(d[d < 1.5])*1./len(d))

        d = np.array(ds_qc[ims])
        eff_qc2.append(len(d[d < 1.5])*1./len(d))

        d = np.array(ds_qc[int(ims*ims/2)])
        eff_qc3.append(len(d[d < 1.5])*1./len(d))

        d = np.array(ds_qc[int(ims*ims/10)])
        eff_qc4.append(len(d[d < 1.5])*1./len(d))

    f, ax = plt.subplots()
    plt.plot(imsize**2,eff_cc, c='black', marker='o', label ="Original")
    ax.set_ylim(0,1.4)
    plt.plot([], [], c='gray', ls ='dashed', label = "{0} efficiency relative to original".format(encoding))
    plt.plot(imsize**2, eff_qc, c='blue',  marker='^', label = "{0} Reco. w/ # shots = # pixels".format(encoding))
    plt.plot(imsize**2, eff_qc3,c='teal',   marker='s', label = "{0} Reco. w/ # shots = (# pixels)/2".format(encoding))
    plt.plot(imsize**2, eff_qc4, c='orange',  marker='o', label = "{0} Reco. w/ # shots = (# pixels)/2".format(encoding))
    plt.plot(imsize**2, eff_qc2,c='red',marker='v', label = encoding + " Reco. w/ # shots = (# pixels)$^{1/2}$")
    plt.plot(imsize**2, np.array(eff_qc)/np.array(eff_cc), c='blue',  marker='^', ls ='dashed')
    plt.plot(imsize**2, np.array(eff_qc3)/np.array(eff_cc),c='teal',   marker='s', ls ='dashed')
    plt.plot(imsize**2, np.array(eff_qc4)/np.array(eff_cc),c='orange',   marker='o', ls ='dashed')
    plt.plot(imsize**2, np.array(eff_qc2)/np.array(eff_cc),c='red',marker='v', ls ='dashed')
    
    '''qc_color = {10:'red', 100:'teal', 1000:'navy', 10000:'dodgerblue'}
    qc_style   = {10:{'marker':'s','ls':'dotted'},
                         100:{'marker':'v','ls':'dashed'},
                        1000:{'marker':'^','ls':'solid'},
                       10000:{'marker':'o','markerfacecolor':'none'}}
    for ns in nshots[::-1]:
        plt.plot(noise_levels,eff_qc[ns],c=qc_color[ns], **qc_style[ns], label ="QC Reco. w/ {0} shots".format(ns))'''
    plt.xlabel("Image size [# of pixels]")
    plt.ylabel("Source ID Efficiency")
    plt.title("Simulated images with injected Gaussian noise {0:.0f}% of max".format(noise*100))
    plt.legend(frameon=False)
    plt.savefig("{1}_eff_vs_imsize_noise{0:.0f}.pdf".format(noise*100, encoding))
    plt.show()
...
#run_many_simulations(  nshots = nshots, noise = 0.1,  nsim = 100)
#eff_vs_noise(noise_levels = [0.005, 0.01, 0.02, 0.05, 0.1, 0.15, 0.2, 0.3, 0.4, 0.5], nshots = nshots, encoding = 'QPIE')
eff_vs_imsize(imsizes,noise=0.1, encoding = 'FRQI')
eff_vs_imsize(imsizes,noise=0.1, encoding = 'QLattice')
